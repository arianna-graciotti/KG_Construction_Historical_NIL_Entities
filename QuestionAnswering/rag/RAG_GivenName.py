...
CONFIG = {
    "INPUT_CSVS": [
        "https://github.com/arianna-graciotti/KG_Construction_Historical_NIL_Entities/blob/291a694d0b0a58119113abf2af9430ac1c5f310b/QuestionAnswering/questions/QA_GivenName_QID.csv",
        "https://github.com/arianna-graciotti/KG_Construction_Historical_NIL_Entities/blob/291a694d0b0a58119113abf2af9430ac1c5f310b/QuestionAnswering/questions/QA_GivenName_nil.csv"
    ],
    "PICKLE_FILE": "",
    "TOP_K": 3,
...
def process_retriever(retriever_type: str, df_in: pd.DataFrame, corpus: List[Tuple[str, str]], csv_basename: str):
    ...
    try:
        ...
        results = []
        # Columns: "sentence", "span", "question" (with "gold givenName" optional)
        for idx, row in tqdm(df_in.iterrows(), total=len(df_in), desc=f"{retriever_type}"):
            question = row["question"]
            ans = row.get("gold givenName", "")
            original_sentence = row["sentence"]
            span = row["span"]
            top_docs = retriever.retrieve(original_sentence, CONFIG["TOP_K"])
            ...
            complete_prompt = (
                "Your task is to determine the given name(s) of a given person occurring in a given sentence based on some provided relevant context. "
